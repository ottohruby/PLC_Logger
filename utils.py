...
def get_date(mode):
    ...
    if mode == "file":
        ...
        raise ValueError(f"Invalid mode selected in utils get_date function: {mode}")
...
def binary_bytes_to_tuple(raw, length, byteorder='little', signed=False):
    ...
    return tuple(int.from_bytes(pair, byteorder=byteorder, signed=signed) for pair in zip(raw[::2], raw[1::2]))
